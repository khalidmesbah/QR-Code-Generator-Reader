...
const unValidateUrlEl = (urlEl) => urlEl.className = `is-invalid form-control`;
...
const getData = () => ({
    ...
    primaryClr: customColorsBtn.checked ? document.getElementById(`primary-color`).value : `#000000`,
    secondaryClr: customColorsBtn.checked ? document.getElementById(`secondary-color`).value : `#ffffff`
});
const generateQrCode = ({ text, size, primaryClr, secondaryClr }) => {
    if (primaryClr === secondaryClr) secondaryClr = getComplementHexColor(secondaryClr);
    try {
